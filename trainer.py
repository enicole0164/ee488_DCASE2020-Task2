...
class Trainer:
    ...
    def train(self, train_loader, valid_loader, save_path):
        ...
        # MEMORY FIX 1: Remove double progress bar - no tqdm on epoch loop
        for epoch in range(self.epochs):
            sum_loss = 0.
            sum_accuracy = 0.
            
            # MEMORY FIX 2: Single progress bar with epoch info
            for i, (x_wavs, x_mels, labels) in tqdm(enumerate(train_loader), 
                                                    total=num_steps, 
                ...
                # MEMORY FIX 3: Clear cache every 50 batches during training
                if (i + 1) % 50 == 0:
            ...
            # MEMORY FIX 4: Clear cache before validation
            torch.cuda.empty_cache()
    ...
    def valid(self, valid_loader):
        ...
        # MEMORY FIX 5: Add torch.no_grad() to prevent gradient computation in validation
        with torch.no_grad():
            for i, (x_wavs, x_mels, labels) in enumerate(valid_loader):
                x_wavs, x_mels, labels = x_wavs.to(self.device), x_mels.to(self.device), labels.to(self.device)
                logits, _ = self.net(x_wavs, x_mels, labels, train=False)
                sum_accuracy += get_accuracy(logits, labels)
                loss = self.criterion(logits, labels)
                ...
                # MEMORY FIX 6: Clear cache every 10 validation batches
                if (i + 1) % 10 == 0:
                    torch.cuda.empty_cache()
    ...
    def test(self, test_loader):
        ...
        sum_accuracy = 0.
        with torch.no_grad():
            for i, (x_wavs, x_mels, labels, AN_N_labels) in enumerate(test_loader):
                x_wavs, x_mels, labels, AN_N_labels = x_wavs.to(self.device), x_mels.to(self.device), labels.to(self.device), AN_N_labels.to(self.device)
                ...
                # MEMORY FIX 7: Clear cache every 20 test batches
                if (i + 1) % 20 == 0:
                    torch.cuda.empty_cache()
