...
class SpecNet(nn.Module):
    ...
    def __init__(self, 
                 ...
                 strides=[2, 2, 2],       # Smaller strides than ASDNet
                 use_log_scale=True,
                 eps=1e-7):
        ...
        self.layers = nn.Sequential(
            # Layer 1: Conv1D + ReLU
            nn.Conv1d(1, channels, kernel_sizes[0], strides[0], 
                     padding=kernel_sizes[0]//2, bias=True),
            nn.ReLU(inplace=True),
            
            # Layer 2: Conv1D + ReLU
            nn.Conv1d(channels, channels, kernel_sizes[1], strides[1], 
                     padding=kernel_sizes[1]//2, bias=True),
            nn.ReLU(inplace=True),
            
            # Layer 3: Conv1D + ReLU
            nn.Conv1d(channels, channels, kernel_sizes[2], strides[2], 
                     padding=kernel_sizes[2]//2, bias=True),
            nn.ReLU(inplace=True)
        )
